                ...
                override fun onAddToFavorites(clickableNode: ClickableNode, intent: Intent?) {
                    if (intent != null) {
                        DialogHelper.confirm(requireActivity(),
                                ...
                                {
                                    val result = ActionShortcutManager(requireContext())
                                            .addShortcut(intent, IconPathAnalysis().loadLogo(requireContext(), clickableNode), clickableNode)
                                    if (!result) {
    ...
    private fun actionExecute(action: ActionNode, onExit: Runnable) {
        ...
        if (action.params != null) {
            ...
            if (actionParamInfos.size > 0) {
                ...
                val handler = Handler()
                progressBarDialog.showDialog(requireContext().getString(R.string.onloading))
                Thread(Runnable {
